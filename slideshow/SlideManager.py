...
class SlideManager:
    ...
    def addSlide(self, file):
        ...
        if isinstance(file, dict) and "file" in file:
            filename = file["file"]
            logger.debug("is a dict>>>>>: %s", file)

        if isinstance(filename, str):
            logger.debug("is a file >>>>> %s", file)
        ...
        if len(background_audio) > 0:
            ...
            if len(background_sections) > 0:
                ...
                for i, section in enumerate(background_sections):
                    ...
                    filter_chains.append("[b%s]afade=t=in:st=%s:d=%s:curve=nofade,afade=t=out:st=%s:d=%s:curve=nofade[b%sf]" % (
